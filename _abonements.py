...
def request(client_id):
    try:
        ...
        for i, j in zip(contracts, contracts_code):
            ...
            try:
                abonement_price_goal = float(
                    abonement_list[9][1].split('из ')[1].split(',00')[0].strip().replace(' ', ''))

            except:
                abonement_price_goal = 0.0

            try:
                abonement_price_current = float(
                    abonement_list[9][1].split('из ')[0].split(',00')[0].strip().replace(' ', ''))
            ...
            # abonement_price = 0.0
            abonement_price = max(abonement_price_sum, abonement_price_current, abonement_price_goal,
                                  abonement_price_rofl)
